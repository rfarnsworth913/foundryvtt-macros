/* ==========================================================================
    ...
    Source:             https://github.com/kandashi/Macros/blob/master/Spell%20Macros/Cantrip/Shillelagh.js
    Usage:              DAE ItemMacro
   ========================================================================== */
...
(async () => {
    ...
    if (props.state === "on") {
        ...
        const weapons = props.target.items.filter((item) => {
            return item.data.type === "weapon" &&
                   (item.data.data.baseItem === "quarterstaff" || item.data.data.baseItem === "club");
        });
        let weaponsList = "";
        ...
        // Weapon Dialog ------------------------------------------------------
        new Dialog({
            title: "Choose a club of quarterstaff",
            content: `
            ...
            buttons: {
                ok: {
                    ...
                    callback: async (html) => {
                        ...
                        DAE.setFlag(props.target, `shillelagh`, {
                            id: itemID,
                            damage: weaponCopy.data.damage.parts[0][0],
                            magic:  weaponCopy.data.properties.mgc
                        });

                        let damage    = weaponCopy.data.damage.parts[0][0];
                        let newDamage = damage.replace(/1d(4|6)/g, "1d8");
                        weaponCopy.data.damage.parts[0][0] = newDamage;
    ...
    if (props.state === "off") {
        let flag = DAE.getFlag(props.target, `shillelagh`);
        ...
        props.target.updateEmbeddedDocuments("Item", [weaponCopy]);
        DAE.unsetFlag(props.target, `shillelagh`);
