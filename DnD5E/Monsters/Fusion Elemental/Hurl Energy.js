...
(async () => {
    ...
    if (props.state === "OnUse" && props.pass === "preDamageRoll") {
        ...
        await props.item.data.data.damage.parts.push(["2d8 + @mod", randomElement]);
    }

    if (props.state === "OnUse" && props.pass === "postActiveEffects") {
        // Setup basic Hurl Elements ability
        // Determine elemental type
        // Get target and source tokens
        // Create animation handler (generic if possible)
        // Specific animation handling
