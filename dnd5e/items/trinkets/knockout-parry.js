...
async function getItems ({ actorData } = {}) {
    ...
    // Initial filtering ------------------------------------------------------
    const weapons = actorData.items.filter((item) => {
        return item.type === "weapon" && item.system.actionType === "mwak";
    });
...
async function applyUpdates (uuid) {
    ...
    // Setup tracking flag ----------------------------------------------------
    DAE.setFlag(props.caster.actorData, "knockoutParry", {
    ...
    const newDiceCount = itemCopy.system.damage.parts[0][0].split("d")[0] + 1;

    // 3. Add effect application
    // 4. Local testing
    // 5. Item testing
    ...
    updates.embedded.Item[itemCopy.name] = {
        ...updates.embedded.Item[itemCopy.name],
        "system.damage.parts": [[
            `${itemCopy.system.damage.parts[0][0].replace(/^[1-9]/, newDiceCount)}`,
            itemCopy.system.damage.parts[0][1]
        ]],
        "flags.midiProperties.fulldam": true,
...
async function removeUpdates ({ actorData } = {}) {
    ...
    // Get dependencies -------------------------------------------------------
    const flag = DAE.getFlag(actorData, "knockoutParry");
    ...
    DAE.unsetFlag(actorData, "knockoutParry");
